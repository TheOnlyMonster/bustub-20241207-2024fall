...
auto BufferPoolManager::NewPage() -> page_id_t {
  ...
  disk_scheduler_->IncreaseDiskSpace(pid + 1);
  return pid;
}
/**
...
auto BufferPoolManager::DeletePage(page_id_t page_id) -> bool {
  ...
  const auto frame_id = page_table_.at(page_id);
  ...
  if (frame->is_dirty_) {
    ...
    DiskRequest dr{true, frame->GetDataMut(), page_id, std::move(promise)};
...
auto BufferPoolManager::CheckedWritePage(page_id_t page_id, AccessType access_type) -> std::optional<WritePageGuard> {
  std::shared_ptr<FrameHeader> frame;
  {
    ...
    if (page_table_.count(page_id) != 0U) {
      auto target_frame_id = page_table_.at(page_id);
      frame = frames_.at(target_frame_id);
      replacer_->RecordAccess(target_frame_id, access_type);
      replacer_->SetEvictable(target_frame_id, false);
      frame->pin_count_.fetch_add(1);
    } else {
      frame_id_t victim_frame_id;
      if (!free_frames_.empty()) {
        victim_frame_id = free_frames_.back();
        free_frames_.pop_back();
      } else {
        const auto evicted_frame_id = replacer_->Evict();
        if (!evicted_frame_id.has_value()) {
          return std::nullopt;
        }
        victim_frame_id = evicted_frame_id.value();
        for (auto &[evicted_page_id, fid] : page_table_) {
          if (fid == victim_frame_id) {
            auto evicted_frame = frames_.at(victim_frame_id);
            if (evicted_frame->is_dirty_) {
              std::promise<bool> promise;
              auto future = promise.get_future();
              DiskRequest dr{true, evicted_frame->GetDataMut(), evicted_page_id, std::move(promise)};
              disk_scheduler_->Schedule(std::move(dr));
              future.get();
      ...
      frame = frames_.at(victim_frame_id);
      frame->Reset();
      std::promise<bool> promise;
      auto future = promise.get_future();
      DiskRequest dr{false, frame->GetDataMut(), page_id, std::move(promise)};
      disk_scheduler_->Schedule(std::move(dr));
      if (!future.get()) {
      ...
      replacer_->RecordAccess(victim_frame_id, access_type);
